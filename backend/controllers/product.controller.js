...
export const getFeaturedProducts = async (req,res) => {
    ...
    //If in mongodb, store in redis for future quick access
...
export const deleteProduct = async (req,res) =>{
    try {
        const product = await Product.findById(req.params.id); // we use .id because it is what we set up in product route
if(!product){
...
if (product.image){
    ...
    try {
        await cloudinary.uploader.destroy(`products/${publicId}`); // folder location is products
...
async function updateFeaturedProductsCache() {
    try {
      ...
      await redis.set("featured_products", JSON.stringify(featuredProducts));
        ...
        console.log("Error in update cache functiom");
    }
}